...
export const Profile = () => {
  ...
  /**
   * useEffect: Al montar el componente, carga citas y foto desde localStorage
   */
  useEffect(() => {
    cargarCitas();
    cargarFoto();
  ...
  /**
   * Carga las citas guardadas en localStorage key: "citas"
   */
  const cargarCitas = () => {
  ...
  /**
   * Elimina una cita por ID y actualiza localStorage
   * @param id - Identificador numérico de la cita
   */
  ...
  const cambiarFoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      setFoto(fotoDataUrl);
      localStorage.setItem("fotoPerfil", fotoDataUrl);
      // Actualizar también el usuario en el contexto de autenticación
      actualizarUsuario({ ...usuario, photoURL: fotoDataUrl });
    };
    reader.readAsDataURL(file);
  };
